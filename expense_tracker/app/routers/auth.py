...
@router.post("/signup")
async def signup(user: UserSchema, session: AsyncSession = Depends(get_session)) -> UserOutputSchema:
    return await create_user(user, session)


@router.post("/login", response_model=dict)
async def login(user_credentials: UserLoginSchema, session: AsyncSession = Depends(get_session)):
